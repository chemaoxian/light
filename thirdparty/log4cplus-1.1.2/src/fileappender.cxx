...
DailyRollingFileAppender::init(DailyRollingFileSchedule sch)
{
    ...
    nextRolloverTime = calculateNextRolloverTime(Time::gettimeofday());
}
...
DailyRollingFileAppender::calculateNextRolloverTime(const Time& t) const
{
    ...
    case WEEKLY:
        return (t + Time(7 * 24 * 60 * 60 - (t.getTime() % (7 * 24 * 60 * 60))));
    ...
    case DAILY:
        return (t + Time(24 * 60 * 60 - (local_now.tm_hour*3600+local_now.tm_min*60+local_now.tm_sec)));

    case TWICE_DAILY:
        return (t + Time(12 * 60 * 60 - (local_now.tm_hour*1800+local_now.tm_min*60+local_now.tm_sec)));

    case HOURLY:
        return (t + Time(60 * 60 - (local_now.tm_min*60+local_now.tm_sec)));

    case MINUTELY:
        return (t + Time(60 - local_now.tm_sec));
    };
}
